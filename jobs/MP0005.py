            ...
            # Find the <Ldap> element
                ...
                if enable_element is not None and server_address_element is not None:
                    if enable_element.text == "true" and server_address_element.text != "0.0.0.0":
                        ...
                        logFile.write(f"\033[91mSUCCESS\033[0m:0:The LDAP service is enabled:{target}:{ports}:{data1}:{user_name_element.text}:{server_auth_element.text}:{auth_method_element.text}::\n")
                    else:
